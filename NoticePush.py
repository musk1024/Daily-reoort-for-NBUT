...
# PushDeer推送
def Push_Deer(title,content):
    if not GlobalVariable.PUSHDEERKEY:
        print("pushdeer推送的PUSHDEER未设置!!")
        return
    res = requests.get(
        f"https://api2.pushdeer.com/message/push?pushkey={GlobalVariable.PUSHDEERKEY}&text={title}&desp={content}")
    if res.status_code == 200:
        print("PushDeer推送成功!")
    else:
        print("PushDeer推送失败!")
